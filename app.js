...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Display current date for exchange rate
  if (rateDateElement) {
    ...
    rateDateElement.textContent = `🗓️ Tasa actual al ${formattedDate}`;
  }
...
// Drag functionality for install button
installBtn.addEventListener('mousedown', startDrag);
installBtn.addEventListener('touchstart', startDrag, { passive: false });
...
function startDrag(e) {
  ...
  let startLeft = parseInt(button.style.left) || 0;
  let startTop = parseInt(button.style.top) || 0;

  function moveHandler(e) {
    const currentX = isTouch ? e.touches[0].clientX : e.clientX;
    const currentY = isTouch ? e.touches[0].clientY : e.clientY;
    
    button.style.left = `${startLeft + currentX - startX}px`;
    button.style.top = `${startTop + currentY - startY}px`;
  }
